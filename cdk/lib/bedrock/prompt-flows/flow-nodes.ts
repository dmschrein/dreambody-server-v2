// cdk/lib/bedrock/prompt-flows/flow-nodes.ts
...
import { PromptVariant } from "@cdklabs/generative-ai-cdk-constructs/lib/cdk-lib/bedrock";
...
export class FlowNode {
  ...
  public static prompt(props: PromptNodeProps) {
    return new FlowNode({
      ...
      // include all versions of the prompt
      neededPolicyStatements: [
        new PolicyStatement({
  ...
  _computeConnections() {
    return this.inputs?.flatMap((input) => {
      return FlowConnection.data({
        ...
        config: {
          sourceOutput:
            input.valueFrom.sourceOutputName ??
            input.valueFrom.sourceNode.outputs[0].name,
