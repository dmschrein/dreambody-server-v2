// cdk/lib/bedrock/prompt-flows/flow.ts
...
import { FlowAlias, FlowAliasProps } from "./flow-alias";
...
export interface FlowProps {
   ...
   */
  readonly substitutions?: { [key: string]: string | number | boolean };
  /**
...
export class Flow extends Resource implements IFlow {
  ...
  constructor(scope: Construct, id: string, props: FlowProps) {
    ...
    // CFN Props
    // ------------------------------------------------------
    const { definition, definitionS3Location, definitionString } =
      props.definition;
  ...
  // ------------------------------------------------------
  /**
   * Create a version for the Flow.
   * @param description The description of the version.
   * @returns The Flow version.
   */
  public createVersion(description?: string): string {
    const flowVersion = new FlowVersion(
      this,
      `FlowVersion-${this._hash.slice(0, 16)}`,
      {
        flow: this,
